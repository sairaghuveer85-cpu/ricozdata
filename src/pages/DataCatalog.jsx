    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        ...
        <button
          ...
          className="flex lg:hidden items-center justify-center gap-2 px-3.5 py-2 rounded-md bg-white dark:bg-[#0D1828] border border-slate-200/80 dark:border-[#1D3047] text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-[#111C2E] cursor-pointer"
        >
          <Filter className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" />
          <span>Filters</span>
          {activeFilterCount > 0 && (
            <span className="px-1.5 py-0.2 rounded text-[10px] bg-blue-100 dark:bg-blue-900/60 text-blue-700 dark:text-blue-300 font-bold tabular-nums">
              {activeFilterCount}
            </span>
          )}
        </button>
      ...
      {(activeFilterCount > 0 || searchTerm.trim()) && (
        <div className="flex items-center flex-wrap gap-1.5 py-1 text-xs">
          ...
          {selectedSources.map((source) => (
            <span
              key={source}
              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-slate-100 dark:bg-[#111E30] text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-[#1D3047]"
            >
              <span>Source: {source}</span>
              <button
                type="button"
                onClick={() => setSelectedSources(selectedSources.filter((s) => s !== source))}
                className="hover:text-slate-900 dark:hover:text-white cursor-pointer p-0.5 rounded focus-visible:outline-2 focus-visible:outline-blue-500"
          ...
          {selectedQualityTier !== 'All' && (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-slate-100 dark:bg-[#111E30] text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-[#1D3047]">
          ...
          <button
            ...
            className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-semibold px-2 py-0.5 cursor-pointer underline-offset-2 hover:underline ml-1"
          >
            Clear all
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs text-slate-500 dark:text-slate-400 px-1">
            <span>
              Showing <strong className="text-slate-900 dark:text-white tabular-nums">{filteredDatasets.length}</strong> of{' '}
              <strong className="text-slate-900 dark:text-white tabular-nums">1,248</strong> enterprise datasets
            </span>
